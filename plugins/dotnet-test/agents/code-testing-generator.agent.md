...
>
> **Literal spec tokens (graded)**: Before declaring done, list every distinctive token in the user's spec — concrete numbers (`500ms`, `7`), filenames (`testdir.zip`), function/symbol names, adjectives or example values (`"such as space"` → the literal space char), and any quoted strings. `grep` your new test file for each one. Every token must appear literally in an input or expected value. Paraphrasing (`100ms` for `500ms`, `'a'` for a space example, `defrag` for `defragment`) silently fails the rubric — the grader checks for the literal token, not the concept.
